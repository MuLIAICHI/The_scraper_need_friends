...
Button.click()
sleep(2)
# Cretae the DataFrame 
df = pd.DataFrame(columns=['job_title', 'partnership', 'company'])
# Get the data from the first  pages 
for p in range(4):
    ...
    for box in Boxes:
        ...
        df_ = df.append({"job_title": title, "partnership": partnership, "company": company}, ignore_index=True)
        # write the data into the csv file 
    df_.to_csv('data_pd.csv', index=False)
